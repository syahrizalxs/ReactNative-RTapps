...
const DetailKeluhan = ({navigation, route}) => {
  useEffect(() => {
    getDetail()
  }, [])
  ...
  return (
    <ScrollView contentContainerStyle={{flexGrow: 1}}>
      ...
      <ActivityIndicator color={colors.default} style={style.loading} size="large" animating={loading} />
        ...
        <Text style={style.label}>Nama Pelapor</Text>
        <Text style={style.value}>{form.pelapor}</Text>
        <View style={style.space(20)}></View>
        <Text style={style.label}>Judul Keluhan</Text>
        <Text style={style.value}>{form.judulKeluhan}</Text>
        <View style={style.space(20)}></View>
        <Text style={style.label}>Deskripsi Keluhan</Text>
        <Text style={style.value}>{form.deskripsiKeluhan}</Text>
        <View style={style.space(20)}></View>
        <Text style={style.label}>Lokasi Kejadian</Text>
        <Text style={style.value}>{form.lokasiKejadian}</Text>
        <View style={style.space(20)}></View>
        <Text style={style.label}>Tanggal Lapor</Text>
        <Text style={style.value}>{new Date(form.tanggalBertamu._seconds * 1000).toLocaleDateString("id-ID")}</Text>
      </View>
    </ScrollView>
  ) 
}
